...
class VideoRecordingManager:
    """
    Manages 10-second video recordings for detections
    """
    ...
    def start_recording(self, camera, alert_type, confidence, detection_results, annotated_frame):
        """Start a 10-second video recording for a detection"""
        try:
    ...
    def _record_video_thread(self, camera_id, recording_info):
        """Thread function to record 10 seconds of video"""
        try:
    ...
    def _create_alert_after_recording(self, recording_info):
        ...
        try:
            ...
            if camera.user.role == 'admin':
                ...
                description = f"Test detection of {alert_type.replace('_', ' ')} with {confidence:.2f} confidence. 10-second video recorded with detection highlights."
            else:
                title = f"{alert_type.replace('_', ' ').title()} Detection Alert"
                description = f"Detected {alert_type.replace('_', ' ')} on camera {camera.name} with {confidence:.2f} confidence. 10-second video recorded for review."
            ...
            with transaction.atomic():
                alert = Alert.objects.create(
                    ...
                    notes=f"10-second video recorded. Unique ID: {unique_id}. Frames: {recording_info['frames_recorded']}"
                )
...
class EnhancedDetectionManager:
    ...
    with proper alert storage and 10-second video recording
    """
    ...
    def __init__(self):
        ...
        self.recording_manager = VideoRecordingManager()
    ...
    def start(self):
        ...
        logger.info("Starting Enhanced Detection Manager with Video File Priority and 10-second Recording")
    ...
    def create_alert_with_recording(self, camera, alert_type, confidence, detection_results, annotated_frame):
        """Create alert with 10-second video recording"""
        try:
            ...
            if recording_result:
                logger.info(f"Started 10-second recording for {alert_type} detection on camera {camera.id}")
...
class EnhancedCameraProcessor:
    ...
    def _process_frame(self, frame, fps, width, height):
        ...
        try:
            ...
            #     try:
            #         annotated_frame = self.choking_detector.process_frame(frame)
            #         frame_diff = cv2.absdiff(frame, annotated_frame)
            #         diff_sum = np.sum(frame_diff)
            #         print("=======================> ", diff_sum)

            #         if diff_sum > 100000:  # Threshold for detecting annotations
    ...
    def _save_detection_video(self, alert_type, confidence, detection_results, annotated_frame, fps, width, height):
        try:
            ...
            frames_to_record = fps * 10  # 10 seconds
            frames_recorded = 0
    ...
    def _create_detection_alert(self, alert_type, confidence, detection_results, video_path, thumbnail_path, frames_recorded, unique_id):
        try:
            ...
            description = f"{self.camera.name} detection of {alert_type.replace('_', ' ')} with {confidence:.2f} confidence. 10-second video recorded."
            with transaction.atomic():
                alert = Alert.objects.create(
                    title=title,
                    description=description,
                    ...
                    thumbnail=thumbnail_relative_path,
                    status='pending_review',
                    notes=f"{self.camera.name} 10-second video. Unique ID: {self.camera.id}. Frames: {frames_recorded}"
                )
